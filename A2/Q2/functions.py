...
def convert_to_gspan_input(input_file, output_path_class0, output_path_class1):
    """
    ...
    Returns:
    - order (list): List of labels for each graph.
    """
    ...
    with open(output_path_class0, 'w') as output_file_class0, open(output_path_class1, 'w') as output_file_class1:
        ...
        for line in lines:
            if line.startswith('#'):
                ...
                output_file = output_file_class0 if label == 0 else output_file_class1
...
def process_subgraph_file(subgraph_file, top_k=50):
    """
...
def generate_feature_vectors(subgraphs, n):
    feature_vectors = np.zeros((len(subgraphs), n), dtype=int)  # Initialize a 2D array with zeros
